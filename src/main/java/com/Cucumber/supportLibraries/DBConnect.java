...
public class DBConnect {
	...
	public static String Getclienthealthinfo(String Usrname , String RecordDate) {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:oracle:thin:@10.128.91.61:1521:hkpes", "cis8uata", "manusit1");
			// step3 create the statement object
			Statement stmt = con.createStatement();

			// step4 execute query
			ResultSet rs = stmt.executeQuery(
					"select * from manuclub_client_health_info where club_id in (select club_id from manuclub_client_profile where username like '%"+ Usrname +"%') and record_date = '"+ RecordDate +"' order by record_date desc");
			while (rs.next())
